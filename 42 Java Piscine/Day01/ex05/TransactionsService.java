...
public class TransactionsService {
    private final UsersList userDataBase;
    ...
    public int retrieveUserBalance(int userIdentifier) throws UserNotFoundException {
        User user = userDataBase.findByIdentifier(userIdentifier);
        return user.getBalance();
    }

    public Transaction[] retrieveUserTransaction(int userIdentifier) throws UserNotFoundException {
        User user = userDataBase.findByIdentifier(userIdentifier);
        return user.getTransactionsList().toArray();
    }

    public void removeTransactionFromUser(int userIdentifier, UUID transactionIdentifier) throws TransactionNotFoundException, UserNotFoundException {
        User user = userDataBase.findByIdentifier(userIdentifier);
        user.getTransactionsList().removeTransactionByIdentifier(transactionIdentifier);
    }

    public void transferTransaction(int senderIdentifier, int recipientIdentifier, int transferAmount)  throws IllegalTransactionException, UserNotFoundException {
